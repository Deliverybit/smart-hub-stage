...
if not agreed:
    ...
    scanned_count = min(SCREENER_SYMBOL_LIMIT, len(NASDAQ_UNIVERSE))
    ...
    if not results:
        ...
        if showing_closest:
            st.info(
                ...
                "so showing the closest available NASDAQ stocks instead."
            )
        else:
            st.success(f"Found **{len(df)}** candidates from {scanned_count} of {len(NASDAQ_UNIVERSE)} stocks scanned.")
