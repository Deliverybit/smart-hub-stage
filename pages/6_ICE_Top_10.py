...
agreed = st.session_state.get("agree_terms_ice", False)
...
st.markdown(
    "Screens **{}** ICE-traded commodity futures and commodity ETFs for "
    ...
    "remains available on the Search page.".format(len(ICE_UNIVERSE))
)
...
with col_b:
    st.info(
        "**How it works:** Each commodity future or ETF is checked for (1) proximity to its "
...
if not agreed:
    ...
    scanned_count = min(SCREENER_SYMBOL_LIMIT, len(ICE_UNIVERSE))
    ...
    if not results:
        ...
        if showing_closest:
            st.info(
                f"No ICE commodities are within **{threshold}%** of their 52-week low right now, "
                "so showing the closest available ICE commodities instead."
            )
        else:
            st.success(f"Found **{len(df)}** candidates from {scanned_count} of {len(ICE_UNIVERSE)} ICE instruments scanned.")
