"""
Archived Search page — full backup of the ticker dashboard (app.py as of 2026-08-06).

Not served directly. To restore to navigation:
  1. Copy or merge into pages/_Analyze.py (live hidden route) or app.py (home).
  2. Re-add st.sidebar.page_link(...) for Search on all pages.

Run archived copy standalone (dev only):
  python -m streamlit run archived/search_page.py
"""
...
if "ticker" in st.query_params:
    _qp_raw = st.query_params.get("ticker", "")
    if isinstance(_qp_raw, list):
        _qp_raw = _qp_raw[0] if _qp_raw else ""
    _qp_ticker = str(_qp_raw).strip().upper()
    if _qp_ticker:
        st.session_state["sidebar_ticker"] = _qp_ticker
